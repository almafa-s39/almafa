# Debian

> [!NOTE]
> Used version: Debian 13.3

## AAA

- [FreeRADIUS configuration](aaa/general.md)
- [FreeRADIUS - LDAP auth](aaa/rad-ldap.md)
- [FreeRADIUS - MySQL queries](aaa/rad-sql.md)
- [FreeRADIUS LDAP domain Proxy](vpn/rad-domain-proxy.md)

## Database

- [MariaDB](db/mariadb.md)
- [PHPMyAdmin](db/phpmyadmin.md)
- [Redis](db/redis.md) <!-- NEW - WS2017 körül-->

## DHCP

- [DHCP-RELAY](dhcp/dhcp-relay.md)
- [ISC DHCPv4](dhcp/isc-dhcp-server.md)
- [ISC DHCPv6](dhcp/isc-dhcp6-server.md)
- [KEA DHCPv4](dhcp/kea.md)
- [KEA DHCPv6](dhcp/kea6.md)

## Directory services

- [LDAP Login, and share automount](directory/ldap-share-mount.md)
- [Local user restriction](directory/local-user-restriction.md)
- [OpenLDAP](directory/ldap.md)
- [Samba LDAP authentication](directory/smb-ldap-auth.md)
- [Samba shares (custom home)](directory/smb-shares.md)

## Disk management, file management, file shares

- [GlusterFS](shares/glusterfs.md)
- [Lsyncd](shares/lsyncd.md)
- [Rsync](shares/rsync.md) <!-- NEW -->
- [LVM](disk/lvm.md)
- [NFS](shares/nfs.md)
- [ProFTPD](shares/proftpd.md)
- [Quota](disk/quota.md)
- [RAID](disk/mdadm.md)
- [VsFTPD](shares/vsftpd.md)
- [ZFS](disk/zfs.md)

## DNS

- [Bind9](dns/bind9.md)
- [DNS Unbound](dns/dns-unbound.md)
- [DNS64](dns/dns64.md)
- [DNSSEC](dns/dnssec.md)
- [Email security](dns/email-security.md)
- [Zone operations](dns/operations.md)

## GUI related configurations

- [GNOME login message](gui/gnome-login-message.md)

## Mailing

- [Email basic authentication with LMTP](mail/basic-auth.md)
- [Email-LDAP authentication](mail/ldap.md)
- [Email security records](dns/email-security.md)
- [Postfix echo](mail/postfix-echo.md)
- [Roundcube webmail](mail/roundcube.md)
- [Levlista, Alias, Vacation] <!-- NEW -->

## Monitoring

- [Cacti](monitoring/cacti.md)
- [Icinga2](monitoring/icinga.md)
- [SNMP](monitoring/snmp.md)
- [Zabbix](monitoring/zabbix.md)

## Network

- [IPTables](network/iptables.md)
- [NFTables](network/nftables.md)
- [PBR](network/pbr.md)
- [VLAN subinterface] <!-- NEW -->

## PKI

- [CA](pki/ca.md)
- [Revocation](pki/revocation.md)
- [SSH Cert](pki/ssh-cert.md)

## Syslog

- [Syslog-ng](syslog/syslog-ng.md)

## VPN

### OpenVPN

- [OpenVPN - General - RemoteAccess](vpn/ovpn-ra-general.md)
- [OpenVPN - General - S2S](vpn/ovpn-s2s-general.md)
- [OpenVPN - FreeRADIUS - LDAP](vpn/ovpn-rad-ldap.md)
- [OpenVPN - FreeRADIUS - MySQL (Low budget ISE)](vpn/ovpn-ise.md)
- [OpenVPN - FreeRADIUS PAM](vpn/ovpn-rad-pam.md)

### StrongSwan

- [Remote Access Cert](vpn/strongswan-ra-cert.md)
- [Remote Access EAP-TLS](vpn/strongswan-ra-eap-tls.md)
- [Remote Access PSK](vpn/strongswan-ra-psk.md)
- [S2S Policy based](vpn/strongswan-pbased.md)
- [S2S Route based](vpn/strongswan-rbased.md)

### WireGuard

- [WireGuard Remote Access](vpn/wireguard-ra.md)
- [WireGuard S2S](vpn/wireguard-s2s.md)

## VRRP

- [Keepalived](vrrp/keepalived.md)

## Web

### Proxy

- [HAProxy reverse proxy](web/haproxy.md)
- [Nginx reverse proxy](web/nginx-proxy.md)
- [Squid](web/squid.md)

### Web hosting

- [Apache2](web/apache2.md)
- [Nginx](web/nginx.md)
- [Roundcube webmail](mail/roundcube.md)
